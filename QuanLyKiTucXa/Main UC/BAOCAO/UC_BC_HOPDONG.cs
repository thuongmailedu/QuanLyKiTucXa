...
{
    public partial class UC_BC_HOPDONG : UserControl
    {
        string connectionString = "Data Source=LAPTOP-MGOO2M8J\\SQLEXPRESS07;Initial Catalog=KL_KTX;Integrated Security=True";

        public UC_BC_HOPDONG()
        {
            InitializeComponent();
        }

        private void UC_BC_HOPDONG_Load(object sender, EventArgs e)
        {
            try
            {
                // Load danh sách nhà
        ...
        private void btnXemBaoCao_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // Load dữ liệu
                DataTable dtBaoCao = GetDataBaoCao(maNha, maPhong, tuNgay, denNgay);

                if (dtBaoCao.Rows.Count == 0)
                {
                    MessageBox.Show($"Không có hợp đồng mới từ {tuNgay:dd/MM/yyyy} đến {denNgay:dd/MM/yyyy}!",
                        "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }

                // Hiển thị báo cáo
                HienThiBaoCao(dtBaoCao, maNha, maPhong, tuNgay, denNgay, tenNV);
            }
            catch (Exception ex)
            {
        ...
        private DataTable GetDataBaoCao(string maNha, string maPhong, DateTime tuNgay, DateTime denNgay)
        {
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                SqlCommand cmd = new SqlCommand("sp_BaoCaoHopDongMoi", conn);
        ...
        private void HienThiBaoCao(DataTable data, string maNha, string maPhong, DateTime tuNgay, DateTime denNgay, string tenNV)
        {
            try
            {
                // Sử dụng Embedded Resource
                reportViewer1.LocalReport.ReportEmbeddedResource = "QuanLyKiTucXa.ReportsSystem.Reports.rptHOPDONG.rdlc";
                reportViewer1.LocalReport.DataSources.Clear();

                // Gán DataSource
                ReportDataSource rds = new ReportDataSource("DataSet_HOPDONG", data);
                ...
                ReportParameter[] parameters = new ReportParameter[]
                {
                    ...
                    new ReportParameter("prTU", tuNgay.ToString("dd/MM/yyyy")),
                    new ReportParameter("prDEN", denNgay.ToString("dd/MM/yyyy")),
