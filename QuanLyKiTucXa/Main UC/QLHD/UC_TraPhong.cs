    ...
    public partial class UC_TraPhong : UserControl
    {
        ...
        private void btnExport_Click(object sender, EventArgs e)
        {
            ...
            if (dgvTraPhong.SelectedRows.Count > 0)
            {
                ...
                foreach (DataGridViewRow row in dgvTraPhong.SelectedRows)
                {
                    if (!row.IsNewRow)
            ...
            else
            {
                ...
                foreach (DataGridViewRow row in dgvTraPhong.Rows)
                {
                    if (!row.IsNewRow)
                        rowsToExport.Add(row);
                }
            ...
            SaveFileDialog saveDialog = new SaveFileDialog
            {
                ...
                FileName = $"DanhSachTraPhong_{DateTime.Now:ddMMyyyy_HHmmss}"
            };
            ...
            try
            {
                ...
                worksheet.Name = "Danh sách trả phòng";

                // ✅ THÊM HEADER (Dòng 1)
                int colIndex = 1;
                foreach (DataGridViewColumn col in dgvTraPhong.Columns)
                {
                    if (col.Visible)
                    {
                        ...
                        headerCell.Interior.Color = System.Drawing.ColorTranslator.ToOle(Color.LightCoral);
